/*Дан двумерный массив размером n×m, заполненный случайными числами. 
Определить, есть ли в данном массиве строка, в которой имеется ровно два максимальных элемента всего массива.
· Если есть, то заменить все симметричные относительно главной диагонали элементы на 
нули, а после этого вставить перед всеми строками, первый элемент которых делится на n, 
строку из нулей.
· Если же в массиве нет строки, в которой имеется ровно два максимальных элемента всего массива, 
то удалить столбец, в котором находится первый (при просмотре элементов построчно сверху направо) 
четный отрицательный элемент и поменять местами средние столбцы (если столбцов четное количество) со вторым и предпоследним*/
